...
impl<'hs> CharSearcher<'hs> {
    pub(crate) fn new(haystack: Bytes<'hs>, chr: char) -> Self {
        ...
        if R::USE_EARLY_REJECT {
            self.state
                .next_reject_fwd(self.haystack.as_bytes(), |bytes| {
                    let width = Bytes(bytes).advance_range_start(0..bytes.len());
        ...
        if R::USE_EARLY_REJECT {
            self.state
                .next_reject_bwd(self.haystack.as_bytes(), |bytes| {
                    let width = bytes.len() - Bytes(bytes).advance_range_end(0..bytes.len());
...
unsafe impl<'hs> Searcher<Bytes<'hs>> for CharSearcher<'hs> {
