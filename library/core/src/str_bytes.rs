...
use crate::ops;
use crate::pattern;
use crate::pattern::{Haystack, MatchOnly, Pattern, RejectOnly, SearchStep, Searcher};
use crate::slice::pattern::NeedleSearcherState;
use crate::str::{
...
pub struct CharSearcher<'hs, F> {
    haystack: Bytes<'hs, F>,
    state: NeedleSearcherState<CharBuffer>,
}

#[derive(Clone, Debug)]
struct CharBuffer([u8; 4], u8);

impl CharBuffer {
    fn new(chr: char) -> Self {
        let mut buf = [0; 4];
        let len = chr.encode_utf8(&mut buf).len() as u8;
        Self(buf, len)
    }
}

impl<'hs> crate::slice::pattern::Needle<'hs, u8> for CharBuffer {
    fn len(&self) -> usize {
        self.1 as usize
    }
    fn test(&mut self, slice: &'hs [u8]) -> bool {
        // SAFETY: self.1 is guaranteed to be valid length, see `new`
        // constructor.
        let needle = unsafe { self.0.get_unchecked(..self.1 as usize) };
        slice == needle
    }
}
...
impl<'hs, F: Flavour> CharSearcher<'hs, F> {
    ...
    pub fn new(haystack: Bytes<'hs, F>, chr: char) -> Self {
        let chr = CharBuffer::new(chr);
        let state = NeedleSearcherState::new(haystack.len(), chr);
        Self { haystack, state }
    }
    ...
    fn next_fwd<R: SearchResult>(&mut self) -> R {
        if R::USE_EARLY_REJECT {
            self.state
                .next_reject_fwd(self.haystack.as_bytes(), |bytes| {
                    let width = Bytes::from(bytes).advance_range_start(0..bytes.len());
                    // SAFETY: advance_range_start always introduces a non-zero
                    // offset.
                    unsafe { crate::num::NonZeroUsize::new_unchecked(width) }
                })
                .map_or(R::DONE, |(s, e)| R::rejecting(s, e).unwrap())
        } else {
            // Note: We’re not calling adjust_reject_start_bwd because
            // 1) RejectOnly case is handled above,
            // 2) MatchOnly case doesn’t return rejects and matches are always
            //    found at valid split positions and
            // 3) in SearchStop case rejects will always start at old start
            //    position and end at the next match or end of haystack which is
            //    yet to be searched.
            self.state.next_fwd::<R, _>(self.haystack.as_bytes())
        }
    }
    ...
    fn next_bwd<R: SearchResult>(&mut self) -> R {
        if R::USE_EARLY_REJECT {
            self.state
                .next_reject_bwd(self.haystack.as_bytes(), |bytes| {
                    let width = bytes.len() - Bytes::from(bytes).advance_range_end(0..bytes.len());
                    // SAFETY: advance_range_end always introduces a non-zero
                    // offset.
                    unsafe { crate::num::NonZeroUsize::new_unchecked(width) }
                })
                .map_or(R::DONE, |(s, e)| R::rejecting(s, e).unwrap())
        } else {
            // Note: We’re not calling adjust_reject_end_fwd because
            // 1) RejectOnly case is handled above,
            // 2) MatchOnly case doesn’t return rejects and matches are always
            //    found at valid split positions and
            // 3) in SearchStop case rejects will always end at old end position
            //    and start at the next match or start of haystack which is yet
            //    to be searched.
            self.state.next_bwd::<R, _>(self.haystack.as_bytes())
        }
    }
}

unsafe impl<'hs, F: Flavour> Searcher<Bytes<'hs, F>> for CharSearcher<'hs, F> {
...
impl<'hs, 'p, F: Flavour> Pattern<Bytes<'hs, F>> for &'p str {
    ...
    fn is_contained_in(self, haystack: Bytes<'hs, F>) -> bool {
        haystack.as_bytes().contains(self.as_bytes())
    }
...
enum StrSearcherInner<'p> {
    ...
    Char(NeedleSearcherState<CharBuffer>),
...
impl<'p> StrSearcherInner<'p> {
    fn new<F: Flavour>(haystack: Bytes<'_, F>, needle: &'p str) -> Self {
        match needle.len() {
            0 => Self::Empty(EmptySearcherState::new(haystack)),
            1 => {
                let chr = CharBuffer::new(needle.chars().next().unwrap());
                Self::Char(NeedleSearcherState::new(haystack.len(), chr))
            }
            _ => Self::Str(StrSearcherState::new(haystack, needle)),
        }
    }
    ...
    fn next_fwd<R: SearchResult, F: Flavour>(&mut self, haystack: Bytes<'_, F>) -> R {
        match self {
            ...
            Self::Char(state) => state.next_fwd::<R, _>(haystack.as_bytes()),
    ...
    fn next_bwd<R: SearchResult, F: Flavour>(&mut self, haystack: Bytes<'_, F>) -> R {
        match self {
            ...
            Self::Char(state) => state.next_bwd::<R, _>(haystack.as_bytes()),
